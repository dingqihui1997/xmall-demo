  ...
  <div class="box flex-a">
    ...
    <div class="right">
      ...
      <div class="quantity flex">
        ...
        <div><input type="text" v-model.number="sum" /></div>
        <div
          class="Symbol jia"
          @click="add"
          :class="{ hover: sum === list.limitNum }"
        >
          +
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    add() {
      if (this.sum < this.list.limitNum) {
        ...
        this.$message.warning("该宝贝不能购买更多哦");
      }
    },
  },
  mounted() {
  ...
  watch: {
    sum(val, oldval) {
      ...
      //   console.log(val);
      if (!/(^[1-9]\d*$)/.test(val)) {
        //当用户输入中文时，就赋值input框为空的
        this.sum = null;
      } else {
        if (val > this.list.limitNum) {
          this.sum = this.list.limitNum;
          this.$message.warning(`该宝贝最多只能购买${this.list.limitNum}件`);
          //   //   return;
          //   setTimeout(() => {
          //     // this.sum = oldval;
          //   }, 100); //间隔执行
